...
*******************************************************************************/
    bool SampleMgr::IsFake() {
        return _vtx.isFake[_idx];
    }
    
    bool SampleMgr::IsOfflinePV() {
        return ( _vtx.Type[_idx] == 1 );
    }
    
    bool SampleMgr::passNdof() {
        return ( _vtx.Ndof[_idx] > 4 );
    }
    
    bool SampleMgr::passAbsZ() {
        return ( fabs( _vtx.z[_idx] ) < 24 );
    }
    
    bool SampleMgr::passRho() {
...
*   HLT selection
*******************************************************************************/
    bool SampleMgr::passHLT( const vector<int>& hlt ) {
