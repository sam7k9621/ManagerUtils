...
namespace mgr{
    ...
    int SampleMgr::MatchGenlevel( const float& eta, const float& phi ) {
    ...
    int SampleMgr::GetGenLep(const int& idx){
        return MatchGenlevel( _lep.GenEta[idx], _lep.GenPhi[idx] );
    }
    
    int SampleMgr::GetGenJet(const int& idx){
        return MatchGenlevel( _jet.Eta[idx], _jet.Phi[idx] );
    }
    
    int SampleMgr::GetDirectMother( int idx ) {
        if( _gen.Mo1[idx] < 0 ) {
            return -1;
        }
    ...
    int SampleMgr::GetGenPdgID(const int& idx){
        return _gen.PdgID[ idx ];
    }
    
    int SampleMgr::GetLepCharge( const int& idx ){
        return _lep.Charge[idx];
    }
   
    int SampleMgr::GetDirectMotherPdgID(const int& idx ){
        return GetGenPdgID( GetDirectMother( idx ) );
    }

    bool SampleMgr::HasCommonMo(const int& idx1, const int& idx2, const int& pdgid){
    ...
    bool SampleMgr::MCTruthBJet(vector<int>& bhandle){
